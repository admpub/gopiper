...
func init() {
	RegisterFilter("required", required, "非空", `required`, ``)
	RegisterFilter("email", email, "E-mail地址", `email`, ``)
	RegisterFilter("username", username, "用户名(字母/数字/汉字)", `username`, ``)
	RegisterFilter("singleline", singleline, "单行文本", `singleline`, ``)
	RegisterFilter("mutiline", mutiline, "多行文本", `mutiline`, ``)
	RegisterFilter("url", url, "URL", `url`, ``)
	RegisterFilter("chinese", chinese, "全是汉字", `chinese`, ``)
	RegisterFilter("haschinese", haschinese, "包含汉字", `haschinese`, ``)
	RegisterFilter("minsize", minsize, "最小长度", `minsize`, ``)
	RegisterFilter("maxsize", maxsize, "最大长度", `maxsize`, ``)
	RegisterFilter("size", size, "匹配长度", `size(5)`, ``)
	RegisterFilter("alpha", alpha, "字母", `alpha`, ``)
	RegisterFilter("alphanum", alphanum, "字母或数字", `alphanum`, ``)
	RegisterFilter("numeric", numeric, "纯数字", `numeric`, ``)
	RegisterFilter("match", match, "正则匹配", `match([a-z]+)`, ``)
	RegisterFilter("unmatch", unmatch, "正则不匹配", `unmatch([a-z]+)`, ``)
	RegisterFilter("match2", match2, "正则匹配(兼容Perl5和.NET)", `match([a-z]+)`, ``)
	RegisterFilter("unmatch2", unmatch2, "正则不匹配(兼容Perl5和.NET)", `unmatch([a-z]+)`, ``)
}
